...
from utils.utils_datasets_augment import TrainSetDataLoader, MultiTestSetDataLoader
...
def main(args):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.batch_size * len(gpus_list),
        shuffle=True,
    )
    ...
    net = torch.nn.DataParallel(
        net.to(device), device_ids=gpus_list, output_device=gpus_list[0]
    )
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=args.n_steps, gamma=args.gamma
    )
    ...
    for idx_epoch in range(start_epoch, args.epoch):
        ...
        logger.log_string(
            "The %dth Train, loss is: %.5f, psnr is %.5f, ssim is %.5f"
        ...
        if args.local_rank == 0:
            save_ckpt_path = str(
                checkpoints_dir
            ) + "/%s_%dx%d_%dx_epoch_%02d_model.pth" % (
                args.model_name,
                args.angRes_in,
                args.angRes_in,
                args.scale_factor,
            ...
            logger.log_string(
                "Saving the epoch_%02d model at %s" % (idx_epoch + 1, save_ckpt_path)
            )

        """ Validation """
        step = 5
        if (idx_epoch + 1) % step == 1 or idx_epoch > args.epoch - 10:
        # if True:
            with torch.no_grad():
                ...
                for index, test_name in enumerate(test_Names):
                    ...
                    psnr_iter_test, ssim_iter_test, LF_name = test(
                        test_loader, device, net, save_dir
                    )
                    excel_file.write_sheet(
                        test_name, LF_name, psnr_iter_test, ssim_iter_test
                    )
                    ...
                    logger.log_string(
                        "The %dth Test on %s, psnr/ssim is %.3f/%.4f"
                ...
                logger.log_string(
                    "The mean psnr on testsets is %.5f, mean ssim is %.5f"
...
def test(test_loader, device, net, save_dir=None):
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(
        ...
        subLFin = LFdivide(
            Lr_SAI_y, args.angRes_in, args.patch_size_for_test, args.stride_for_test
        )
        ...
        subLFout = rearrange(
            subLFout, "(n1 n2) 1 a1h a2w -> n1 n2 a1h a2w", n1=numU, n2=numV
        )
        ...
        if save_dir is not None:
            ...
            Sr_SAI_rgb = (
                ycbcr2rgb(Sr_SAI_ycbcr.squeeze().permute(1, 2, 0).numpy()).clip(0, 1)
                * 255
            ).astype("uint8")
            Sr_4D_rgb = rearrange(
                Sr_SAI_rgb,
                "(a1 h) (a2 w) c -> a1 a2 h w c",
                a1=args.angRes_out,
                a2=args.angRes_out,
            )
