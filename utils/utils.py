...
def get_logger(log_dir, args):
    ...
    logger = logging.getLogger(args.model_name+'_'+str(datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")))
...
def create_dir(args):
    ...
    log_dir.mkdir(exist_ok=True)
    if args.augment == 'maskblur':
        augment_name = '{}_prob{}_mask_ratio{}_mask_patch{}_drop{}'.format(args.augment, args.prob, args.mask_ratio, args.mask_patch, args.drop_prob)
        print(augment_name)
    else:
        augment_name = args.augment
    log_dir = log_dir.joinpath(augment_name)
    log_dir.mkdir(exist_ok=True)
        ...
        for u in range(U):
            for v in range(V):
                PSNR[b, u, v] = metrics.peak_signal_noise_ratio(
                    label_y[b, u, :, v, :].numpy(), out_y[b, u, :, v, :].numpy(), data_range=1.0
                )
                if args.task == "RE":
...
def rgb2gray(x):
    ...
    y = np.zeros(x.shape, dtype="double")
    y[:, :, 0] = 255 * 0.2989 * x[:, :, 0] + 255 * 0.5870 * x[:, :, 1] + 255 * 0.1140 * x[:, :, 2] + 16.0
    y = y / 255.0
    return y
