    ...
    def __init__(self, args):
        ...
        self.AngFE = nn.Sequential(
            nn.Conv2d(
                1,
                channels,
                kernel_size=int(self.angRes),
                stride=int(self.angRes),
                padding=0,
                bias=False,
            )
        )
        ...
        self.CascadeInterBlock = CascadeInterBlock(
            self.angRes, n_groups, n_blocks, channels
        )
...
class make_chains(nn.Module):
    def __init__(self, angRes, channels):
        ...
        self.Spa2Ang = nn.Conv2d(
            channels,
            channels,
            kernel_size=int(angRes),
            stride=int(angRes),
            padding=0,
            bias=False,
        )
        ...
        self.AngConvSq = nn.Conv2d(
            2 * channels, channels, kernel_size=1, stride=1, padding=0, bias=False
        )
        self.SpaConvSq = nn.Conv2d(
            2 * channels,
            channels,
            kernel_size=3,
            stride=1,
...
class ReconBlock(nn.Module):
    def __init__(self, angRes, channels, upscale_factor):
        ...
        self.FinalConv = nn.Conv2d(
            int(channels), 1, kernel_size=1, stride=1, padding=0, bias=False
        )
        self.angRes = angRes
