...
class get_model(nn.Module):
    ...
    def forward(self, lf_lr, angRes):
        ...
        [B, _, H, W] = lf_lr.size()
        lf_lr = lf_lr.view(
            B, 1, self.angRes, H // self.angRes, self.angRes, W // self.angRes
        ).permute(0, 1, 2, 4, 3, 5)
        lf_lr = lf_lr.contiguous().view(
            B, self.an2, H // self.angRes, W // self.angRes
        )  # [B, angRes**2, h, w]
        B, an2, h, w = lf_lr.size()
        ######################################################

        # bilinear
        lf_lr_upsample = functional.interpolate(
            lf_lr, scale_factor=self.scale, mode="bilinear", align_corners=False
        )

        # individual LR feature extraction 对子图像做特征提取，子图像之间不做交互
        lf_fea = self.relu(self.fea_conv0(lf_lr.view(-1, 1, h, w)))
        lf_fea = self.fea_resblock(lf_fea).view(
            B, an2, -1, h, w
        )  # [B,an2,channels,h,w]
        ...
        for i in range(self.an2):
            # 构造 pair feature
            lf_pair_fea = torch.cat(
                [lf_fea, lf_fea[:, i : i + 1, :, :, :].repeat(1, an2, 1, 1, 1)], dim=2
            )  # [N,an2,128,h, w]

            # pair fusion 将所有视图与参考视图的特征融合
            lf_pair_fea = self.relu(
                self.pair_conv0(lf_pair_fea.view(B * an2, -1, h, w))
            )
            lf_pair_fea = self.pair_resblock(lf_pair_fea)
            lf_fea_aligned = self.pair_conv1(lf_pair_fea)  # [N*an2,64,h, w]
            lf_fea_aligned = lf_fea_aligned.view(
                B, an2, -1, h, w
            )  # [N, an2, channels, h, w]

            # all view fusion 将所有视图做特征融合 到一个视图
            lf_fea_aligned = torch.transpose(
                lf_fea_aligned, 1, 2
            ).contiguous()  # [B, channels, an2,h,w]
            ref_fea_fused = self.relu(
                self.fusion_view_conv0(lf_fea_aligned.view(-1, an2, h, w))
            )  # [N*channels,64,h,w]
            ref_fea_fused = self.fusion_view_resblock(
                ref_fea_fused
            )  # [B*channels,64,h,w]
            ref_fea_fused = self.relu(
                self.fusion_view_conv1(ref_fea_fused)
            )  # [B*channels,1,h,w]
            ref_fea_fused = ref_fea_fused.view(B, -1, h, w)  # [B,channels,h,w]
            ref_fea_fused = self.relu(
                self.fusion_fea_conv0(ref_fea_fused)
            )  # [B,channels,h,w]
            ...
            out_one_view = self.conv_last(
                out_one_view
            )  # [B,1,h*scale_factor,w*scale_factor]
        ...
        # reshape for output
        out = out.view(
            B, self.angRes, self.angRes, h * self.scale, w * self.scale
        ).permute(0, 1, 3, 2, 4)
        out = out.contiguous().view(
            B, 1, self.angRes * h * self.scale, self.angRes * w * self.scale
        )
...
class AltFilter(nn.Module):
    def __init__(self, an):
        ...
        self.an = an
        self.relu = nn.ReLU(inplace=True)
        self.spaconv = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1
        )
        self.angconv = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1
        )
