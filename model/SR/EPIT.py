...
class get_model(nn.Module):
    ...
    def forward(self, lr, info=None):
        # Bicubic
        lr_upscale = interpolate(lr,
                                 self.angRes,
                                 scale_factor=self.factor,
                                 mode='bicubic')
        # Initial Feature Extraction
        lr = rearrange(lr,
                       'b c (a1 h) (a2 w) -> b c (a1 a2) h w',
                       a1=self.angRes,
                       a2=self.angRes)
...
class BasicTrans(nn.Module):
    ...
    def __init__(self, channels, spa_dim, num_heads=8, dropout=0.):
        ...
        self.attention = nn.MultiheadAttention(spa_dim,
                                               num_heads,
                                               dropout,
                                               bias=False)
        ...
        self.feed_forward = nn.Sequential(
            nn.LayerNorm(spa_dim), nn.Linear(spa_dim, spa_dim * 2, bias=False),
            nn.ReLU(True), nn.Dropout(dropout),
            nn.Linear(spa_dim * 2, spa_dim, bias=False), nn.Dropout(dropout))
    ...
    def gen_mask(self, h: int, w: int, k_h: int, k_w: int):
        ...
        attn_mask = attn_mask.float().masked_fill(
            attn_mask == 0, float('-inf')).masked_fill(attn_mask == 1,
                                                       float(0.0))
    ...
    def forward(self, buffer):
        ...
        attn_mask = self.gen_mask(v, w, self.mask_field[0],
                                  self.mask_field[1]).to(buffer.device)
...
def interpolate(x, angRes, scale_factor, mode):
    ...
    x_upscale = x_upscale.permute(0, 2, 4, 1, 3,
                                  5).contiguous().view(B * angRes**2, 1, h, w)
    x_upscale = F.interpolate(x_upscale,
                              scale_factor=scale_factor,
                              mode=mode,
                              align_corners=False)
    x_upscale = x_upscale.view(B, angRes, angRes, 1, h * scale_factor,
                               w * scale_factor)
    x_upscale = x_upscale.permute(0, 3, 1, 4, 2, 5).contiguous().view(
        B, 1, H * scale_factor, W * scale_factor)  # [B, 1, A*h*S, A*w*S]
...
if __name__=="__main__":
    ...
    args.scale_factor = 2
    inputs = torch.randn((1, 1, 5 * 32, 5 * 32),device="cuda:0")
    model = get_model(args=args).cuda()
    output = model(inputs)
    print(output.size())
